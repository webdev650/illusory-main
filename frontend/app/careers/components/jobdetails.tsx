...
import { Mail, MapPin, Briefcase, GraduationCap, CheckCircle } from 'lucide-react';
...
export const JobDetailsContent: React.FC<JobDetailsContentProps> = ({ job, onApply }) => {
  return (
    <div className="bg-white text-black font-jakartaSans h-full overflow-y-auto custom-scrollbar">
        ...
        <header className="mb-12 border-b border-gray-100 pb-12">
          <div className="flex justify-between items-start mb-6">
            ...
            {job.isActivelyHiring && (
              <div className="flex items-center gap-1.5 px-3 py-1 border border-green-200 text-[10px] font-bold uppercase tracking-widest text-green-600 bg-green-50 shrink-0">
                <CheckCircle size={12} />
                Actively Hiring
              </div>
            )}
          ...
          <section>
            ...
            <ul className="grid grid-cols-1 gap-4">
              {job.responsibilities.map((item, index) => (
                <li key={index} className="flex gap-4 p-4 border border-gray-100 hover:border-gray-200 transition-colors">
