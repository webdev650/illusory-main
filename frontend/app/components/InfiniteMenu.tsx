...
void main() {
    ...
    vec3 dirOnSphere = centerPos - sphereCenter;
    vAlpha = smoothstep(-0.5, 0.5, dirOnSphere.z / 2.0) * 0.85 + 0.15;
    vUvs = aModelUvs;
  ...
  #initTexture() {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        ctx.save();
        // Sleek dark circular badge for disc item
        ...
        if (img) {
          ...
          let drawW = img.width || cellSize;
          let drawH = img.height || cellSize;
...
export default function InfiniteMenu({ items = [], scale = 1.0 }: InfiniteMenuProps) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      const activeItemsList = items.length ? items : defaultItems;
      const itemIndex = index % activeItemsList.length;
      setActiveItem(activeItemsList[itemIndex]);
    };
